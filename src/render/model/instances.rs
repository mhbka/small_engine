...
use cgmath::{Matrix4, Quaternion, Vector3, prelude::*};
use std::ops::Range;
use wgpu::{
...
impl Instances {
    ...
    pub fn initialize(label: &str, gpu: &GpuContext, instances: Vec<Instance>) -> Self {
        ...
        let buffer =
            GpuBuffer::create_writeable_vertex(label, gpu, bytemuck::cast_slice(&instance_data));
    ...
    pub fn actual(&mut self) -> &mut Vec<Instance> {
        &mut self.instances
    }
